import { DError } from '@/application/enums/errors'
import type { AppRequest } from './request'

import { validatePassword } from '@/lib/utils/validations/auth'
  ...
  public validate(): { message: string; statusCode: DError } | boolean {
    if (!this.email) {
      return {
        ...
        statusCode: DError.EMAIL_REQUIRED,
      }
    }
    ...
    if (!this.password) {
      return {
        ...
        statusCode: DError.PASSWORD_REQUIRED,
      }
    }
    ...
    if (this.password && !validatePassword(this.password)) {
      return {
        ...
        statusCode: DError.INVALID_PASSWORD,
      }
    }
