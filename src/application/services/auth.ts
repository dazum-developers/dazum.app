...
import type { Json } from '@/components/format-message'
...
export async function loginService(
  ...
  User | null | UserNotFoundError | CerealNotFoundError | InvalidPasswordError
> {
  const { email, password } = payload

  async function getUser(
    params: Readonly<{ email: string; password: string }>,
  ) {
  ...
  async function getCereal(
    ...
    const cerealResult: Cereal | null = await getUserCerealById(
      params.userResult?.id as string,
    )
  ...
  async function verifyCredential(
    ...
    if (cereal?.key) {
      let decodePayload: string | Json = Buffer.from(
        cereal?.key,
        'base64',
      ).toString()
